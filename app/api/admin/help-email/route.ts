...
export async function POST(request: NextRequest) {
  try {
    ...
    const subject = '[오도이촌 캠지기] 회원정보 문의 접수'
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color:#2F7D32;">회원정보 문의가 접수되었습니다</h2>
        <div style="background:#f5f5f5;padding:16px;border-radius:8px;">
          <p><strong>캠핑장 이름:</strong> ${campgroundName}</p>
          <p><strong>담당자명:</strong> ${managerName}</p>
          <p><strong>연락처:</strong> ${phone}</p>
          <p><strong>메일 주소:</strong> ${email}</p>
          <p><strong>문의 시각:</strong> ${timestamp}</p>
        </div>
        <p style="color:#666;margin-top:16px;">담당자가 확인 후 회신 드리겠습니다.</p>
      </div>
    `
